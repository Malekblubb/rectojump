...
namespace rj
{
	...
	public:
		level_manager(const std::string& abs_level_path)
			: m_dirh{abs_level_path}, m_abs_path{m_dirh.get_path()}
		{
			this->init();
		}
		...
		bool save_level(const level_packer<packer_mode::pack>& lv,
						const level_id& id)
		{
			if(!m_filemgr.reopen(this->make_path(id),
		...
		auto& get_levels() const noexcept { return m_loaded_levels; }
	...
	private:
		...
		{
			if(!m_filemgr.reopen(path, std::ios::in)) return;
		...
		level_id make_id(const std::string& str) const
		{
			auto result{str};
			if(result.length() < 4 ||
			   result.substr(result.size() - 4, 4) != ".rjl")
				result += ".rjl";
			return result;
		}
	};
