...
namespace rj
{
	...
	public:
		level_packer(const music_data& music, const level_background& bg,
			...
			: m_music{music}, m_data{data}, m_background{bg}, m_info{info}
		{
			this->create_level();
		}
		...
		auto packed_data() const noexcept -> const mlk::data_packet&
		{
	...
	private:
		...
		void make_header()
		{
			level_header header{this->music_size(), this->background_size(),
								this->data_size(), this->info_size(), 0};
			mlk::cnt::append(
		...
		void compress()
		{
			...
			mlk::cmprs::compressor<mlk::cmprs::cmprs_mode::zlib> cmp{
				m_packed_level};
			cmp.pack();
			...
			mlk::cnt::insert_front(
				std::string{reinterpret_cast<char*>(&unpacked_size),
							sizeof unpacked_size},
				m_packed_level);
			m_valid = true;
		}

		int music_size() const noexcept { return m_music.size(); }

		int background_size() const noexcept { return m_background.size(); }

		int data_size() const noexcept { return m_data.size(); }

		int info_size() const noexcept { return m_info.size(); }
	};
}
