...
namespace rj
{
	...
	public:
		level_packer(const mlk::data_packet& data) : m_read_data{data}
		{
			this->create_level();
		}

		const level& get_level() const noexcept { return m_result; }

	private:
		void create_level()
		{
			this->unpack();
			if(!m_valid || m_work_data.size() <= 0) return;
			this->parse();
		}
		...
		void unpack()
		{
			...
			compressor<cmprs_mode::zlib> cmp{
				mlk::data_packet{m_read_data.begin() + 4, m_read_data.end()}};
			cmp.unpack(unpacked_size);
			if(!cmp.is_valid()) return;
			m_work_data = cmp.get();
			m_valid = true;
		}
		...
		void parse()
		{
			auto music_data{mlk::data_packet{
				std::begin(m_work_data) + sizeof(level_header),
				std::begin(m_work_data) + sizeof(level_header) +
					this->music_size()}};
			auto backg_data{mlk::data_packet{
				std::begin(m_work_data) + sizeof(level_header) +
					this->music_size(),
				std::begin(m_work_data) + sizeof(level_header) +
					this->music_size() + this->background_size()}};
			auto level_data{mlk::data_packet{
				std::begin(m_work_data) + sizeof(level_header) +
					this->music_size() + this->background_size(),
				std::begin(m_work_data) + sizeof(level_header) +
					this->music_size() + this->background_size() +
					this->data_size()}};
			auto infos_data{mlk::data_packet{
				std::begin(m_work_data) + sizeof(level_header) +
					this->music_size() + this->background_size() +
					this->data_size(),
				std::begin(m_work_data) + sizeof(level_header) +
					this->music_size() + this->background_size() +
					this->data_size() + this->info_size()}};

			level_parser lv_parser{backg_data, level_data};
			info_parser inf_parser{infos_data};
			m_result = {std::move(music_data), lv_parser.get_bg_result(),
						lv_parser.get_level_result(), inf_parser.get_result()};
		}
