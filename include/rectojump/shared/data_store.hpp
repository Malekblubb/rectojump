...
namespace rj
{
	...
	struct basic_store : public store_base
	{
		...
		basic_store(const map_type& m) : m_values{m} {}
	};
	...
	public:
		...
		{
			auto index{0};
			get_type_index<Data_Type>(
				[&index](int found_index) { index = found_index; },
				m_helpertuple);

			auto ptr{std::static_pointer_cast<basic_store<Data_Type>>(
				m_stores.at(index))};
