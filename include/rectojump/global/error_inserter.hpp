...
namespace rj
{
	...
	public:
		error_inserter(error_handler& eh) : m_errorhandler{eh}
		{
			this->create_list();
			for(auto& a : m_list) mlk::lerr_i().link_error(a);
		}
	...
	private:
		void create_list()
		{
			...
			m_list.emplace_back(errors::io_create_file,
			...
			m_list.emplace_back(errors::cl_nullptr_access,
								"FATAL: access to odd memory.",
								mlk::slot<>{[this] {
									m_errorhandler.exec_error_instance(
										errors::cl_nullptr_access);
								}});
			m_list.emplace_back(
				errors::cl_data, "FATAL: needed data not loaded",
				mlk::slot<>{[this] {
					m_errorhandler.exec_error_instance(errors::cl_data);
				}});
