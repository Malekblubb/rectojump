...
namespace rj
{
	namespace ui
	{
		...
		public:
			...
			{
				auto ptr{
					std::make_shared<Button_Type>(std::forward<Args>(args)...)};
			...
			void inactivate() noexcept { m_current_pressed_index = -1; }
			...
			auto& get_buttons() { return m_buttons; }

		private:
			void draw(sf::RenderTarget& target,
					  sf::RenderStates states) const override
			{
				for(auto& a : m_buttons) target.draw(*a.second.button, states);
			}
		};
	}
}
