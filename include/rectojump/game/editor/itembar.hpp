...
namespace rj
{
	...
	public:
		...
		void update(dur duration) { m_buttons.update(duration); }

		void render() { m_render(m_shape, m_buttons); }

		void select(int index) noexcept { m_buttons.set_active_button(index); }

		void deselect_all() noexcept { m_buttons.inactivate(); }
		...
		{
			return std::static_pointer_cast<button_item>(
					   m_buttons.get_active_btn())
				->get_figure();
		}

		auto get_bounds() const { return m_shape.getGlobalBounds(); }

		const vec2f& get_size() const noexcept { return m_shape.getSize(); }
