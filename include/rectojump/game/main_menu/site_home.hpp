...
namespace rj
{
	...
	public:
		...
		void construct()
		{
            ...
            logo_shape->get().setPosition((m_sites.bounds().width - logo_shape->get().getSize().x) / 2.f,
            ...
            text->get().setPosition((m_sites.bounds().width - text->get().getGlobalBounds().width) / 2.f,
			...
			auto username(m_sites.add_object<ui::textbox>("home",
														  tbsize,
                                                          vec2f{(m_sites.bounds().width - tbsize.x) / 2.f,
                                                                logo_shape->get().getPosition().y + 180.f},
														  font,
														  "Username"));
			auto password(m_sites.add_object<ui::textbox>("home",
														  tbsize,
                                                          vec2f{(m_sites.bounds().width - tbsize.x) / 2.f,
                                                                logo_shape->get().getPosition().y + 220.f},
														  font,
														  "Password"));
