...
namespace rj
{
	...
	public:
		background_manager(rndr& r, Game_Handler& gh)
			: m_render{r}, m_gamehandler{gh}
		{
		}

		void update(dur duration)
		{
			auto do_update_for_state{[this, duration](state s) {
				// update all components of current state
				for(auto& a : m_objects[s].components) a->update(duration);
		...
		void render()
		{
			auto do_render_for_state{[this](state s) {
				// render bg shape
				m_render(m_objects[s].bgshape);

				// update all components of current state
				for(auto& a : m_objects[s].components) a->render();
		...
		{
			return m_objects[m_gamehandler.current_renderable_state()]
				.components.size();
		}

		template <typename Obj_Type, typename... Args>
		mlk::sptr<background_component<Obj_Type>> create_object_for_state(
			state s, Args&&... args)
		{
			auto ptr{std::make_shared<background_component<Obj_Type>>(
				m_render, std::forward<Args>(args)...)};
