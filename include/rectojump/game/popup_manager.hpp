...
namespace rj
{
	...
	public:
		popup_manager(Game_Handler& gh)
			: m_gamehandler{gh},
			  m_font{
				  gh.datamgr().template get_as<sf::Font>(settings::text_font())}
		{
		}

		void update(dur duration)
		{
			for(auto& a : m_popups) a.update(duration);
			this->erase_destroyed();
		}

		void render()
		{
			for(auto& a : m_popups) a.render();
		}
		...
		void create_popup(
			...
			Args&&... args)
		{
			popup_creator<type, Game_Handler>{*this, text, pos,
		...
		{
			m_popups.emplace_back(&m_gamehandler.rendermgr(), m_font,
								  std::forward<Args>(args)...);
		}

		std::size_t num_popups() const noexcept { return m_popups.size(); }
	...
	{
		popup_creator(popup_manager<Game_Handler>& pm, const std::string& text,
					  const vec2f& pos)
		{
			pm.create_popup_impl(text, pos, 4000, to_rgb("#000375", 200),
								 to_rgb("#000352"), to_rgb("#e3e3e3"));
		}
	};
	...
	struct popup_creator<popup_type::error, Game_Handler>
	{
		popup_creator(popup_manager<Game_Handler>& pm, const std::string& text,
					  const vec2f& pos)
		{
			pm.create_popup_impl(text, pos, 4000, to_rgb("#820006", 200),
								 to_rgb("#d80042"), to_rgb("#e3e3e3"));
		}
	};
}
