...
namespace rj
{
	...
	public:
		triangles4(const vec2f& size = {0.f, 0.f},
		...
		void rotate(float angle) { sf::Transformable::rotate(angle); }
		...
		const vec2f& getSize() const noexcept { return m_size; }

		const sf::Color& getFillColor() const noexcept { return m_color; }
	...
	private:
		void init()
		{
			...
			m_verts.append(
				{{m_origin.x + m_size.x, m_origin.y - m_size.y}, m_color});
			m_verts.append(
				{{m_origin.x - m_size.x, m_origin.y - m_size.y}, m_color});
			...
			m_verts.append({m_origin, m_color});
			m_verts.append(
				{{m_origin.x + m_size.y, m_origin.y - m_size.x}, m_color});
			m_verts.append(
				{{m_origin.x + m_size.y, m_origin.y + m_size.x}, m_color});
		...
		void recalculate() noexcept
		{
			...
			m_verts[1].position = {m_origin.x + m_size.x,
								   m_origin.y - m_size.y};
			m_verts[2].position = {m_origin.x - m_size.x,
								   m_origin.y - m_size.y};

			// bottom
			m_verts[3].position = m_origin;
			m_verts[4].position = {m_origin.x + m_size.x,
								   m_origin.y + m_size.y};
			m_verts[5].position = {m_origin.x - m_size.x,
								   m_origin.y + m_size.y};

			// right
			m_verts[6].position = m_origin;
			m_verts[7].position = {m_origin.x + m_size.y,
								   m_origin.y - m_size.x};
			m_verts[8].position = {m_origin.x + m_size.y,
								   m_origin.y + m_size.x};

			// left
			m_verts[9].position = m_origin;
			m_verts[10].position = {m_origin.x - m_size.y,
									m_origin.y - m_size.x};
			m_verts[11].position = {m_origin.x - m_size.y,
									m_origin.y + m_size.x};
		}
		...
		void recolor()
		{
			...
			for(auto iter{begin}; iter != end; ++iter) iter->color = m_color;
		}
